...
- [x] 2026-06-24 -- **Auth: stop re-logging in mid-session (refresh-token rotation).** Backend (`OsuStocks.API`): Redis-backed, single-use **rotating refresh token** (SHA-256 hashed at rest, 30-day sliding TTL), issued on the osu! callback and exchanged at a new `POST /api/v1/auth/refresh` (own per-IP `auth-refresh` limiter; `INVALID_REFRESH_TOKEN` → 401). Frontend: store `refreshToken`/`refreshExpiresAt`, single-flight **silent refresh + one-shot 401 auto-retry** in the API client, a proactive refresh timer (`expiresAt − 2min`) in `AuthProvider`, and session restore-on-load from a valid refresh token. Verified: API build + unit (136) + auth/refresh/rate-limit integration (8) green; FE `tsc` + vitest (10) green.
- [x] 2026-06-24 -- **osu! rate-limiting fixes (fewer 429s).** Backend: routed the OAuth `/oauth/token` client through the existing throttle queue (it was bypassing it — the "queue in the login flow"), added a 429/5xx-aware **retry handler** honoring `Retry-After` (+15s timeouts) on both osu! HTTP clients, **cached the client-credentials token** in Redis (no more token POST every sync minute), **skipped the unused top-score fetch on login**, lowered per-process `OsuApi:RequestsPerMinute` (API 120 / Worker 300 — separate processes were ~1200/min combined), and added a WARN log on throttle-queue rejection.
- [x] 2026-06-24 -- **Subtle AI-use disclosure + em-dash removal.** Added a human-led AI-assistance note to About → Development Team, and removed **every em-dash across the frontend** (150 occurrences in 43 files), rephrasing copy and comments to read naturally (UI "no data" placeholders → plain hyphen). Verified: 0 em/en dashes remain; About page renders clean in preview with no console errors.
- [x] 2026-06-24 -- **Edge hardening at Caddy** (prod, `OsuStocks.API/deploy`). Custom Caddy build (xcaddy: `caddy-ratelimit` + Coraza/OWASP CRS), global **per-IP rate limit** (api 120 / web 240 per min), **1 MB body + 16 KB header caps**, **WAF in DetectionOnly**, and **bad-bot / empty-UA 403s**; compose `caddy` switched `image:`→`build:` and `deploy.sh --caddy` now rebuilds. Prod-only; run `caddy validate` on the server and flip WAF to `On` once logs are clean.
